...
export const signup = async (req, res, next) => {
  try {
    ...
    if (userExists) {
      ...
      // Generate and hashing a random token
      ...
      // Verification URL which is going to be sent to the email of newly signed up user
...
export const userVerified = async (req, res) => {
  try {
    ...
    const hashingUniqueString = crypto
      .createHash("sha256")
      .update(uniqueString)
      .digest("hex");

    const hashedUser = await Verification.findOne({
      uniqueString: hashingUniqueString,
    });
